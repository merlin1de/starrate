...
- Nextcloud läuft unter `https://nextcloud.example.com` (oder `192.168.x.x`)
...
Das Script:
- Setzt Trusted Domains (`localhost`, `nextcloud.example.com`, `192.168.x.x`)
...
Falls `nextcloud.example.com` intern nicht auflöst, in `docker/cypress.yml` diese Zeilen auskommentieren:

```yaml
extra_hosts:
  - "nextcloud.example.com:192.168.x.x"
```
...
docker exec --user www-data nextcloud php occ \
  config:system:set trusted_domains 2 --value=192.168.x.x
```
...
docker compose -f docker/cypress.yml run --rm cypress \
  ...
  --env NC_URL=https://nextcloud.example.com,NC_USER=admin,NC_PASS=xxx
```
