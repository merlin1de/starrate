...
- **Mobile: scroll moved row-by-row instead of pixel-perfect** — even with compression disabled, the virtualized range updated on every row boundary, firing a Vue render + DOM diff that yanked the browser out of compositor-only scroll. The bitmap then advanced in row-sized chunks instead of moving smoothly with the finger. Snap the rendered range to multiples of 4 rows: between snaps the DOM is fully stable, so the GPU keeps moving the bitmap pixel-for-pixel. At each snap a single bigger render fires (4 rows in, 4 rows out instead of 1+1), but four times less often — the larger bursts get masked by touch-momentum frame-coalescing.
...
- **Mobile: Scroll bewegte sich zeilenweise statt pixelperfekt** — auch ohne Compression aktualisierte sich der virtualisierte Render-Range bei jeder Reihen-Grenze, was ein Vue-Render + DOM-Diff auslöste und den Browser aus dem Compositor-only Scroll riss. Die Bitmap rückte dann in Reihen-Schritten weiter statt mit dem Finger pixelperfekt zu folgen. Render-Range auf Vielfache von 4 Reihen gesnappt: zwischen den Snaps ist die DOM komplett stabil, der GPU-Pfad bleibt aktiv und bewegt die Bitmap pixelweise. Pro Snap feuert ein einzelner größerer Render (4 Reihen rein, 4 raus statt 1+1), aber viermal seltener — die größeren Bursts verstecken sich besser im natürlichen Touch-Momentum-Frame-Coalescing.
