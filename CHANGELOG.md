...
- **Mobile scroll: placeholder flashing on slow row-by-row touch scroll** — virtualization buffer was 2 rows above/below the viewport, so a thumb only started loading when its row was already ≤326 px from the viewport. On slow touch-scroll (where momentum doesn't keep buffering busy), the user saw the placeholder shimmer on each entering row. Doubled the buffer to 4 rows (~650 px lead) and bumped parallel thumbnail loads from 5 to 8, so thumbs are usually decoded before their row enters the viewport. IntersectionObserver `rootMargin` raised in lockstep (400 → 800 px).
...
- **Mobile-Scroll: Placeholder-Aufblitzen bei langsamem Zeile-für-Zeile-Touch-Scroll** — der Virtualisierungs-Buffer lag bei 2 Reihen oberhalb/unterhalb des Viewports, also begann der Thumbnail-Load erst, wenn die Zeile ≤326 px vor dem Viewport war. Bei langsamem Touch-Scroll (wo der Momentum-Pool keinen Vorlauf erzeugt) sah man den Placeholder-Shimmer pro Zeile aufblitzen. Buffer auf 4 Reihen verdoppelt (~650 px Vorlauf), parallele Thumbnail-Loads von 5 auf 8 erhöht — Thumbs sind typischerweise dekodiert, bevor ihre Zeile in den Viewport rutscht. `rootMargin` des IntersectionObservers im Gleichschritt von 400 auf 800 px angehoben.
